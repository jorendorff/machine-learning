...
impl Wgpu {
    async fn new(label: Option<&str>) -> Result<Self> {
...
#[tokio::main]
fn main(wgpu: &Wgpu, model: &mut crate::Model) -> anyhow::Result<()> {
    let module = wgpu.load_wgsl_module("adjust.wgsl")?;
    let runner = Runner::new(wgpu, &module)?;
    runner.read_write(0, &mut model.embeddings);
    runner.read_write(1, &mut model.weights);
    runner.read_only(2, &model.paths);
    runner.read_only(3, &model.ranges);
    runner.read_only(4, &model.tasks);
    runner.run().await?;
    Ok(())
}

#[tokio::main]
async fn main2(model: &mut crate::Model) -> anyhow::Result<()> {
    let instance = wgpu::Instance::new(wgpu::InstanceDescriptor {
        backends: wgpu::util::backend_bits_from_env().unwrap_or(wgpu::Backends::PRIMARY),
        dx12_shader_compiler: wgpu::util::dx12_shader_compiler_from_env().unwrap_or_default(),
        gles_minor_version: wgpu::util::gles_minor_version_from_env().unwrap_or_default(),
